...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="absolute bottom-1/4 right-1/3 w-60 h-60 bg-accent-cyan/5 rounded-full blur-3xl" />
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-3xl font-bold text-white">{t("login_title")}</h1>
        ...
        <div className="glass rounded-2xl border border-dark-700 p-8">
          {error && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="flex items-center gap-2 p-3 rounded-xl bg-rose-500/10 border border-rose-500/20 text-sm text-rose-400 mb-6"
            >
              <AlertCircle size={15} />
              {error}
            </motion.div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Link href={`/${locale}/auth/forgot-password`} className="text-xs text-brand-400 hover:text-brand-300">
                  {t("forgot_password")}
                </Link>
              </div>
              <div className="relative">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs text-slate-600">
              <span className="px-3 bg-dark-900">{t("or_continue")}</span>
            </div>
          </div>

          {/* OAuth placeholder */}
          <button className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-dark-800 border border-dark-700 rounded-xl text-sm text-slate-300 hover:border-dark-600 hover:text-white transition-all">
